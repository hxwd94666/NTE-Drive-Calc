...
- 安装包会一并处理虚拟手柄驱动 ViGEmBus，方便扫描模式使用。
...
1. 先打开游戏并进入驱动/卡带仓库页面。
2. 在程序里选择扫描模式。
3. 如果是第一次整理库存，建议使用全量扫描。
4. 如果只是日常新增装备，建议使用增量扫描或半自动截图。
5. 扫描并解析完成后，进入配装页选择角色优先级和分配策略。
6. 确认结果没问题后保存配装，后续更新可以锁定已有方案继续优化。

如果只想看单件装备好不好，可以直接打开“鉴定”，粘贴截图或按快捷键截图后开始鉴定。
